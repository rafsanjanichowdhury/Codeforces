...
int main()
{
    ...
    int n,k; cin>>n>>k; int c1{0},c2{0};
    int arr[26]; fl(i,0,26) arr[i] = 97+i; //sort(arr,arr+n); 
    //fl(i,0,26) cout<<(char)arr[i]<<" ";
    int brr[n]; int j=0;
    fl(i,0,n)
    {
        if(j<k) { brr[i] = arr[j]; j++;}
        else {j=0; brr[i] = arr[j]; j++;}
    }
    //cout<<endl;
    fl(i,0,n) cout<<(char)brr[i];
