...
int main()
{
    ...
    int n,t,k,d; cin>>n>>t>>k>>d; int c1{0},c2{0};
    int f,g;
    n%k ==0 ? f = n/k : f = n/k + 1;
    f = f*t; //total time for single oven

    int Oven_one = (int)(d/t)*k;
    int left_to_make = n - Oven_one;
    left_to_make%(2*k) == 0 ? g = t*(left_to_make/(2*k)) : g = t*(left_to_make/(2*k) + 1);
    if(g+d < f) cout<<"YES"<<endl;
    else cout<<"NO"<<endl;
