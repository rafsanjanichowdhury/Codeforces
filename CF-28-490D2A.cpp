...
int main()
{
    ...
    int n; cin>>n; int c1{0},c2{0},c3{0};
    int arr[n]; 
    fl(i,0,n) 
        {
            cin>>arr[i];
    ...
    int z = min(c1,min(c2,c3)); cout<<z<<endl;;
    while(z--){
        vector <int> temp; int x{1};
        fl(j,0,3){
        fl(i,0,n)
        {
            if(arr[i] == 1 and x==1) {cout<<i+1<<" "; ++x; arr[i]=0;break;}
            else if(arr[i] == 2 and x==2) {cout<<i+1<<" "; ++x; arr[i]=0; break;}
            else if(arr[i] == 3 and x==3) {cout<<i+1<<" "; ++x; arr[i]=0; break;}
        }}
        cout<<endl;
    }
