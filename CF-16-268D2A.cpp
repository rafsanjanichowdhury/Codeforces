...
int main()
{
    ...
    int arr[n][2]; fl(i,0,n) cin>>arr[i][0]>>arr[i][1];
    int brr[4];
    fl(i,0,n){
        fl(j,i+1,n){
            brr[0] = arr[i][0]; brr[1] = arr[i][1];            
            brr[2] = arr[j][0]; brr[3] = arr[j][1];
            if(brr[0] == brr[3] && brr[1] == brr[2]) c1 +=2;
            else if(brr[0] == brr[3] or brr[1] == brr[2]) ++c1;
        }
